...
export default function FocusModePage() {
  ...
  const taskId = useMemo(() => {
    if (paramTaskId && taskStore.getTasks(paramTaskId).length > 0) {
      return paramTaskId;
    }
    const active = ALL_TASKS_INFO.find(t => taskStore.getTasks(t.id).length > 0);
    return active ? active.id : (paramTaskId || '1');
  }, [paramTaskId]);
  ...
  const currentTaskText = tasks[0]?.text || 'No tasks left';
  ...
  const initialTotalSeconds = useMemo(() => {
    ...
    const m = parseInt(minutes || '1', 10); // default 1 min
    ...
    <View style={[styles.container, { backgroundColor: colors.secondary }]}>
      ...
      <View style={[styles.card, { backgroundColor: colors.background }]}>
        ...
        >
          ...
          <Text style={[styles.exitText, { color: colors.textSecondary, fontSize: 12 * scale }]}>
            Exit Focus Mode
          </Text>
        </TouchableOpacity>
      ...
      <TimesUpModal
        ...
        focusDuration={focusDurationString}
