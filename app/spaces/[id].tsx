...
import { MiniRoomTimer } from '@/components/mini-room-timer';
import { PersonAvatar, AvatarState } from '@/components/person-avatar';
...
export default function RoomDetailPage() {
  ...
  const focusState = taskStore.getLastFocusState();
  ...
  const [wasCompletedEarly, setWasCompletedEarly] = useState(false);
  ...
  const nextTask = useMemo(() => {
    return taskStore.getNextUnfinishedTask(taskId);
  }, [taskId, tasks]);

  const handleFinish = (early?: boolean) => {
    setWasCompletedEarly(!!early);
    if (tasks.length <= 1) {
  ...
  const avatars: AvatarState[] = [
    'focus', 'focus', 'idle', 'focus',
    'focus', 'not focus', 'focus', 'focus',
    'focus', 'focus', 'focus', 'idle',
    'focus', 'focus', 'not focus', 'focus',
    'idle', 'focus', 'focus', 'focus'
  ];
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.secondary }]} edges={['top', 'bottom']}>
        ...
        <View style={styles.sheetGradient}>
          ...
          <View style={styles.headerRow}>
            <View style={styles.headerTextCol}>
              <Text style={[styles.titleText, { color: colors.text, fontSize: 32 * scale }]}>{roomData.title}</Text>
              <Text style={[styles.peopleText, { color: colors.textSecondary, fontSize: 14 * scale }]}>{roomData.peopleCount} People here</Text>
            </View>
      ...
      {/* Floating Elements (Over Sheet) */}
      <View style={styles.floatingContainer}>
        <MiniRoomTimer style={styles.miniTimer} onFinish={() => handleFinish(false)} />
        <Navbar menu="Spaces" style={styles.navbar} />
      </View>

      <TimesUpModal
        isOpen={isTimesUpOpen}
        onClose={() => setIsTimesUpOpen(false)}
        focusDuration="Focus Session"
        isCompletedEarly={wasCompletedEarly}
        onTakeBreak={() => {
          router.replace('/home' as any);
        }}
        onContinue={() => {
          taskStore.removeFirstTask(taskId);
          router.replace(`/task/list?id=${taskId}` as any);
        }}
      />
      ...
      <FinishedTaskModal
        ...
        onSelectOption={(option) => {
          ...
          if (option === 'start_new') {
            if (nextTask) {
              router.replace(`/task/list?id=${nextTask.id}` as any);
            } else {
              router.replace({
                pathname: '/home',
                params: { openAddTask: 'true' }
              } as any);
            }
          } else if (option === 'take_break') {
            router.replace('/task' as any);
          } else if (option === 'done_today') {
            router.replace('/home' as any);
          }
        }}
      />
...
const styles = StyleSheet.create({
  ...
  sheetContainer: {
    position: 'absolute',
    top: 100, // Matching the y=98 from Figma slightly
    bottom: 0,
    width: '100%',
  ...
  titleText: {
    fontFamily: 'SourceSerifPro-SemiBold', // Or SourceSerifPro-Bold based on available fonts
    fontSize: 32,
    lineHeight: 42,
    color: '#000',
  },
  ...
  miniTimer: {
    marginBottom: 20, // space above navbar
  },
  navbar: {
    position: 'relative', // override default absolute
    bottom: 0,
  }
});
